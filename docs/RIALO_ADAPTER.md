...
Triggerlane implements the Rialo integration only as far as verified access permits. Every validated Composer draft compiles into the same versioned execution intent consumed by the virtual execution target checks. The current internal schema is named `GhostIR` v1 for compatibility with the original codebase; that legacy identifier is not a claim about the current product brand.

## Current Architecture

```text
Live or Guided data -> Trigger configuration -> GhostIR v1 -> internal Sandbox adapter -> virtual ledger
                                                          |
...
- virtual-ledger and Rialo automation adapter boundaries
