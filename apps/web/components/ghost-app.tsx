  ...
  return (
    <aside className="composer-panel">
      ...
      <section className={`commitment-summary ${canCommit ? "" : "invalid"}`} aria-label="Capital commitment preview">
        ...
        <p>{canCommit ? "Reserved only after arming. Released on cancel or expiry." : "Commitment exceeds currently available Sandbox capital."}</p>
      </section>
      <button className="compiler-action" onClick={() => previewCompiler.mutate()} disabled={previewCompiler.isPending}><BracketsCurly size={17} />{previewCompiler.isPending ? "CHECKING..." : "VIEW TECHNICAL CONTRACT"}<span>Advanced · Sandbox ready</span></button>
...
function CompilerPanel({ preview, close }: { preview: CompilerPreview; close: () => void }) {
  ...
  return <section className="compiler-panel">
    ...
    <div className="compiler-targets"><div className="ready"><span>SANDBOX</span><b>{sandbox.status}</b><small>Ledger-backed execution venue</small></div><div><span>RIALO</span><b>{rialo.status.replaceAll("_", " ")}</b><small>No network or toolchain configured</small></div></div>
...
function PortfolioRail({ workspace, price }: { workspace: Workspace; price: number }) {
  ...
  return (
    <aside className="portfolio-rail">
      <div className="rail-section portfolio-total"><span className="eyebrow">SANDBOX EQUITY</span><strong>${money.format(value)}</strong><small>SIMULATED USDC VALUE</small></div>
    ...
    <div className="trade-layout">
      ...
      <main className="market-workspace">
        <section className="market-header">
          ...
          <div className={`execution-mode ${modeIsLive ? "monitor" : ""}`}><i /><span>{modeIsLive ? "MONITORING ONLY" : "SANDBOX EXECUTION"}</span><b>{modeIsLive ? "LIVE DATA" : "DEMO FEED"}</b></div>
        </section>
        ...
        <AnimatePresence>{selected?.status === "FILLED" && <motion.div className="execution-flash" role="status" initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}><span><Check size={24} weight="bold" /></span><div><b>TRIGGER FILLED</b><small>Sandbox settlement committed exactly once</small></div></motion.div>}</AnimatePresence>
      </main>
...
function GhostsView({ workspace }: { workspace: Workspace }) {
  ...
  return (
    <main className="page-view ghosts-command-page">
      <div className="page-title"><div><span className="eyebrow">AUTOMATION COMMAND CENTER</span><h1>Your triggers</h1><p>See what is closest to acting, why each trigger is waiting, and exactly how much sandbox capital it controls.</p></div><a className="new-ghost" href="/trade"><Plus size={17} />BUILD A TRIGGER</a></div>
...
function PortfolioView({ workspace }: { workspace: Workspace }) {
  ...
  return (
    <main className="page-view portfolio-page">
      <div className="page-title portfolio-title"><div><span className="eyebrow">CAPITAL CONTROL</span><h1>Your sandbox portfolio</h1><p>See what you own, what remains available, and exactly which trigger controls every reserved amount.</p></div><a className="new-ghost" href="/trade"><Plus size={17} />BUILD A TRIGGER</a></div>
      ...
      <section className="portfolio-overview" aria-label="Portfolio overview">
        <div className="equity-statement"><span>TOTAL SIMULATED EQUITY</span><strong>${money.format(equity)}</strong><p>{quantity.format(Number(sol.quantity))} SOL plus {quantity.format(Number(usdc.quantity))} USDC, valued on the current stored frame.</p></div>
      ...
      <section className="reservation-section" aria-labelledby="reservation-title">
        ...
        {activeReservations.length ? <div className="reservation-list">{activeReservations.map((reservation) => {
          ...
          const slippageBps = workspace.ghosts.find((ghost) => ghost.id === reservation.ghostId)?.maxSlippageBps ?? 0;
          ...
          return <article className="reservation-row" key={reservation.id}><div><span className="mini-ghost"><BrandIcon size={17} weight="duotone" /></span><span><a href={`/ghost/${reservation.ghostId}`}>{reservation.ghostName}</a><small>{reservation.status} · {reservation.side} {reservation.asset === "SOL" ? "SOL" : "WITH USDC"}</small></span></div><div><span>CONTROLLED NOW</span><b>{quantity.format(Number(reservation.amount))} {reservation.asset}</b><small>${money.format(value)} current value</small></div><div><span>IF IT EXECUTED NOW</span><b>{reservation.side} → {estimatedOutput}</b><small>illustration at frame price + {slippageBps} bps</small></div><a className="inspect-reservation" href={`/ghost/${reservation.ghostId}`} title={`Inspect ${reservation.ghostName}`}><Eye size={18} /></a></article>;
        })}</div> : <div className="portfolio-empty"><BrandIcon size={32} weight="duotone" /><div><h3>No capital is reserved</h3><p>All ${money.format(equity)} remains available. Start a trigger to see its capital assignment here.</p></div><a href="/trade">BUILD A TRIGGER<ArrowRight size={15} /></a></div>}
      </section>
      ...
      <section className="ledger-section" aria-labelledby="ledger-title">
        ...
        <div className="ledger-list">{visibleLedger.map((transaction) => <article className="ledger-row" key={transaction.id}><div className={`ledger-type ledger-${transaction.type.toLowerCase()}`}><span>{transaction.type === "SEED" ? "01" : "TX"}</span><div><b>{transaction.ghostName ?? "Initial sandbox deposit"}</b><small>{transaction.type} · {dateTime(transaction.createdAt)}</small></div></div><div className="ledger-movements">{transaction.entries.map((entry) => <span className={Number(entry.amount) >= 0 ? "credit" : "debit"} key={entry.id}><b>{Number(entry.amount) >= 0 ? "+" : ""}{quantity.format(Number(entry.amount))} {entry.asset}</b><small>{entry.type.replaceAll("_", " ")}</small></span>)}</div><div className="ledger-reference"><span>TRANSACTION</span><b>{transaction.id.slice(0, 12)}</b>{transaction.ghostId ? <a href={`/ghost/${transaction.ghostId}`}>OPEN RECEIPT <ArrowRight size={13} /></a> : <small>PORTFOLIO GENESIS</small>}</div></article>)}</div>
...
function HistoryView({ workspace }: { workspace: Workspace }) {
  ...
  return <main className="page-view history-audit-page"><div className="page-title"><div><span className="eyebrow">EXECUTION AUDIT TRAIL</span><h1>Trigger history</h1><p>See every settlement, stopped intent, and blocked attempt in the order it happened, with the evidence behind each outcome.</p></div><div className="history-total"><span>SIMULATED SETTLED VALUE</span><b>${money.format(settledVolume)}</b><small>{workspace.executions.length} committed settlement{workspace.executions.length === 1 ? "" : "s"}</small></div></div><section className="history-summary" aria-label="History outcome summary">{(["FILLED", "BLOCKED", "CANCELLED", "EXPIRED", "FAILED"] as const).map((item) => <button key={item} className={status === item ? "active" : ""} onClick={() => setStatus(status === item ? "ALL" : item)}><span>{item}</span><b>{outcomes.filter((outcome) => outcome.status === item).length}</b><small>{item === "FILLED" ? "ledger committed" : item === "BLOCKED" ? "capital restored" : item === "FAILED" ? "settlement failed" : "no execution"}</small></button>)}</section><section className="history-controls" aria-label="Filter history"><label>Search<input placeholder="Trigger or outcome" value={search} onChange={(event) => setSearch(event.target.value)} /></label><div className="history-status-filter" role="group" aria-label="Outcome filter">{(["ALL", "FILLED", "BLOCKED", "CANCELLED", "EXPIRED", "FAILED"] as const).map((item) => <button aria-pressed={status === item} className={status === item ? "active" : ""} key={item} onClick={() => setStatus(item)}>{item === "ALL" ? "All outcomes" : item.charAt(0) + item.slice(1).toLowerCase()}</button>)}</div></section>{outcomes.length === 0 ? <div className="empty-state history-empty"><ClockCounterClockwise size={38} /><h2>No outcomes have been recorded</h2><p>Run, stop, or block a trigger in the Demo Feed to create its first audit record.</p><a href="/trade">RUN A TRIGGER<Play size={15} /></a></div> : visible.length === 0 ? <div className="empty-state history-empty"><Database size={36} /><h2>No audit records match</h2><p>Change the outcome filter or search term to return to the full chronology.</p><button onClick={() => { setStatus("ALL"); setSearch(""); }}>SHOW ALL HISTORY</button></div> : <section className="history-ledger" aria-label="Chronological execution ledger"><header><span>OUTCOME</span><span>TRIGGER</span><span>CONDITION BOUNDARY</span><span>SETTLEMENT</span><span>EVIDENCE</span></header>{visible.map((outcome) => {
    ...
    return <button className={`history-audit-row outcome-${outcome.status.toLowerCase()}`} key={outcome.key} onClick={() => open(outcome)}><span className="outcome-mark">{outcome.status === "FILLED" ? <Check size={16} weight="bold" /> : outcome.status === "BLOCKED" ? <ShieldCheck size={17} /> : <X size={16} />}</span><div className="outcome-label"><StatusBadge status={outcome.status} /><small>{dateTime(outcome.at)}</small></div><div className="outcome-ghost"><b>{outcome.name}</b><small>SIMULATED · SOL/USDC</small></div><div><span>CONDITIONS</span><b>{ready}/{evaluations.length} READY</b><small>{outcome.status === "BLOCKED" || outcome.status === "FILLED" ? "one stored qualifying frame" : "last recorded state"}</small></div><div><span>RESULT</span><b>{settlement}</b><small>{outcome.status === "FILLED" ? "owned balances changed" : "owned balances unchanged"}</small></div><div className="outcome-evidence"><span>{outcome.kind === "receipt" ? "RECEIPT" : outcome.kind === "attempt" ? "ATTEMPT RECORD" : "OUTCOME RECORD"}</span><b>{outcome.id.slice(0, 12)}</b><CaretRight size={17} /></div></button>;
  })}</section>}<AnimatePresence>{selected && <motion.div className="modal-backdrop audit-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={close}><motion.div className="receipt-modal audit-modal" role="dialog" aria-modal="true" aria-label={`${selected.name} ${selected.status.toLowerCase()} audit record`} initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 40, opacity: 0 }} onClick={(event) => event.stopPropagation()}>{selected.execution ? <Receipt execution={selected.execution} close={close} /> : selected.attempt ? <BlockedAudit attempt={selected.attempt} close={close} /> : selected.ghost ? <TerminalAudit ghost={selected.ghost} activities={workspace.activities.filter((activity) => activity.ghost_id === selected.ghost!.id)} close={close} /> : null}</motion.div></motion.div>}</AnimatePresence></main>;
}
      ...
      <section className={`detail-observatory ${dataPaused ? "data-blocked" : ""}`} aria-labelledby="ghost-core-heading">
        ...
        <aside className="observatory-brief">
          ...
          <dl className="observatory-facts">
            ...
            <div><dt>Execution</dt><dd>Triggerlane Sandbox</dd></div>
          </dl>
        </aside>
      </section>
...
function ConnectionDrawer({ id, workspace, live, liveError, diagnostics, sourceAge, close }: { id: string; workspace: Workspace; live?: LiveMarket; liveError: boolean; diagnostics?: Diagnostics; sourceAge: string; close: () => void }) {
  ...
  return <motion.aside id={id} className="connection-drawer" role="dialog" aria-modal="true" aria-labelledby="connections-title" initial={{ x: 32, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 32, opacity: 0 }}>
    ...
    <section className="connection-list">
      ...
      <div><span><Lightning size={18} /><i>SANDBOX EXECUTION</i></span><b>{modeIsLive ? "DISABLED" : "SIMULATED"}</b><small>{modeIsLive ? "Live observations cannot settle capital" : `${diagnostics?.executionAttemptsInFlight ?? 0} attempts in flight · ledger-backed`}</small></div>
    </section>
...
export function GhostApp({ view, ghostId }: { view: AppView; ghostId?: string }) {
  ...
  useEffect(() => {
    api("/api/session/anonymous", { method: "POST" }).then(() => setSessionReady(true)).catch((error) => setBootError(error instanceof Error ? error.message : "Sandbox could not start."));
  }, []);
  ...
  useEffect(() => {
    const close = (event: KeyboardEvent) => { if (event.key === "Escape") { setConnectionsOpen(false); setAccountOpen(false); } };
  ...
  if (fatalMessage) return <div className="fatal-state" role="alert"><BrandIcon size={44} /><span className="eyebrow">WORKSPACE CONNECTION FAILED</span><h1>Sandbox unavailable</h1><p>{fatalMessage}</p><div className="capital-safe"><ShieldCheck size={17} /><span><b>No capital was moved.</b> Your persisted Sandbox remains unchanged.</span></div><button onClick={() => window.location.reload()}>RETRY CONNECTION</button></div>;
    ...
    <AppShell>
      <header className="app-header">
        ...
        <div className="header-tools">
          <div className="mode-switch" role="group" aria-label="Market data mode"><button aria-pressed={!modeIsLive} className={!modeIsLive ? "active" : ""} onClick={() => setMode.mutate("DEMO")}>DEMO</button><button aria-pressed={modeIsLive} className={modeIsLive ? "active" : ""} onClick={() => setMode.mutate("LIVE")}>LIVE</button></div>
          <button className={`connection-pill ${modeIsLive ? "monitoring" : ""}`} aria-expanded={connectionsOpen} aria-controls="connection-drawer" onClick={() => { setConnectionsOpen((value) => !value); setAccountOpen(false); }}><i />{modeIsLive ? "LIVE MONITORING" : "DEMO EXECUTION"}<CaretRight size={14} /></button>
          <button className="account-button" aria-expanded={accountOpen} aria-controls="account-popover" onClick={() => { setAccountOpen((value) => !value); setConnectionsOpen(false); }}><span>{workspace.identity.label.slice(-4)}</span><CaretRight size={14} /></button>
        </div>
      </header>
      ...
      <AnimatePresence>{accountOpen && <motion.div id="account-popover" className="popover account" role="dialog" aria-modal="false" aria-label="Sandbox account" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}><span className="eyebrow">SANDBOX ACCOUNT</span><strong>{workspace.identity.label}</strong><small>{workspace.identity.id.slice(0, 18)}...</small><div className="account-balances"><span>USDC <b>{quantity.format(Number(workspace.portfolio.balances.USDC.quantity))}</b></span><span>SOL <b>{quantity.format(Number(workspace.portfolio.balances.SOL.quantity))}</b></span></div><button onClick={() => { setAccountOpen(false); setConnectionsOpen(true); }}><Broadcast size={16} />CONNECTIONS</button><button onClick={() => clearSession.mutate()}><Power size={16} />CLEAR SESSION</button></motion.div>}</AnimatePresence>
