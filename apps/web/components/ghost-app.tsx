    ...
    <AppShell>
      <header className="app-header">
        ...
        <div className="header-tools">
          ...
          <button className={`connection-pill ${modeIsLive ? "monitoring" : ""}`} aria-expanded={connectionsOpen} aria-controls="connection-drawer" onClick={() => { setConnectionsOpen((value) => !value); setAccountOpen(false); }}><i />{modeIsLive ? "LIVE MONITORING" : "SIMULATED EXECUTION"}<CaretRight size={14} /></button>
          <button className="account-button" aria-label="Open simulation profile" aria-expanded={accountOpen} aria-controls="account-popover" onClick={() => { setAccountOpen((value) => !value); setConnectionsOpen(false); }}><UserCircle size={17} /><span className="account-label">SIM PROFILE</span><CaretRight size={14} /></button>
        </div>
      </header>
